PN_DelayDf_Stack <- function(data = reviewtable, split.interval = ' - '){
  ...
  DelaySplit2 <- DelaySplit2 %>%
    ...
    mutate(Y = case_when(max(id) == 1 ~ 3153600/2,
                         max(id) != 1 ~ ((id-1)*(3153600/(NEntry-1)))))
    
  
  delay <- reshape2::melt(DelaySplit2, id.vars = c('id', 'Entry_ID','Task_type', 'Y'),
  ...
  delay1 <- reshape(delay1, idvar = c('id','Entry_ID','Task_type','Y','ID_Delay'),
