...
ui = navbarPage(id = 'wholePage',
                # App Title
                title=div(img(src="MPIB_Logo_EN_horizontal_RGB_White.png", height = "110px",width = "464px")),
                ...
                tabPanel('ABOUT', id='about',
                         fluidRow(class='aboutbox',
                           ...
                           column(8, 
                                  ...
                                  fluidRow(box(solidHeader = T, status = "warning", width = NULL, 
                                               h3(strong("Overall procedure", style = "color: #006c66")),
                                               p("A schematic pipeline of the five steps: (i) identification of 
                                               relevant papers via PubMed and hand search strategies,
                                               (ii) the screening procedure based on a-priori inclusion and exclusion criteria, 
                                               (iii) the selection of relevant papers, (v) the application of the coding scheme on 
                                               every task entry (observation units) from the selected papers, 
                                               (vi) the visualization approach for each task entry and a bird-eye view aggregated map 
                                               of all task entries using hierarchical edge bundling.", style = "font-size: 18px;"),
                                               img(src="Figure1_shiny.jpg", width = '700px')
                                               
                                               )),
                                  
                                  fluidRow(box(solidHeader = T, status = "warning", width = NULL, 
                                               h3(strong("Visualizing a single task entry", style = "color: #006c66")),
                                               p("Hierarchical edge bundling plots that visualize the combination of methodological variables for each task entry (A). 
                                                 Hierarchical edge bundling plots depicting the proportion of task entries in each design variable (node size), 
                                                 and the density in the combinations between two given design variables (color intensity of connecting lines between nodes) in the whole corpus (B), 
                                                 and in the top 20% of the densest inter-node connections (C).", style = "font-size: 18px;"),
                                               img(src="Fig1.png", width = '1000px')
                                               
                                  )),
                                  
                                  fluidRow(includeHTML(("end_text.html")))
                                  ),
                           column(2)
                         )
                ),
                ...
                tabPanel('OVERVIEW',
                         fluidRow(class='overview',
                           ...
                           column(8, 
                                  p(),
                                  fluidRow(box(solidHeader = T, status = "warning", width = NULL,
                                               h3(strong("Publications over year", style = "color: #006c66")),
                                  ...
                                  fluidRow(box(solidHeader = T, status = "warning", width = NULL,
                                               h3(strong("Locations of data collection", style = "color: #006c66")),
                                               ...
                                               plotlyOutput("illustration_WM", height = 'auto', width = 'auto')%>% withSpinner(type=4,color="#006c66"))),
                                  
                                  fluidRow(box(solidHeader = T, status = "warning", width = NULL,
                                               h3(strong("Overall literature composition", style = "color: #006c66")),
                                               ...
                                               # column(12, align='center',img(src="Figure3.jpg", width = '700px'))
                                               plotOutput('illustration_pie') %>% withSpinner(type=4,color="#006c66")
                                               )),
                                    ...
                                    dashboardBody(
                                      ...
                                      fluidRow(class='exploreSearch',
                                        box(title = NULL, solidHeader = TRUE, status = 'warning', width = NULL,
                                            ...
                                            h3(strong("Search engine", style = "color: #006c66")),
                                            ### instruction ----
                                            ...
                                            fluidRow(
                                              box(title = NULL, solidHeader = TRUE, status = 'warning', width = NULL,
                                                  h4(strong("Search result", style = "color: #006c66")),
                                      ...
                                      fluidRow(
                                        box(title = NULL, solidHeader = T, status = 'warning', width = NULL,
                                            h3(strong("Hierarchical Edge Bundling", style = "color: #006c66")), 
                                            fluidRow(
                                              column(8, align='left',
                                      ...
                                      fluidRow(
                                        box(title = NULL, solidHeader = T, status = 'warning', width = NULL, 
                                            h3(strong("Delay", style = "color: #006c66")),
            ...
            .aboutbox .box.box-solid{background: #ffffff; border-bottom-color:#ffffff; border-left-color:#ffffff; border-right-color:#ffffff; border-top-color:#ffffff;}
...
server <- function(input, output, session) { 
  ...
  output$illustration_year <- renderPlotly({
    ggplotly(reviewtable %>%
               ...
               scale_y_continuous(breaks = function(x) int_breaks(x, n = 10))+ scale_x_continuous(breaks = function(x) int_breaks(x, n = 10))+
  ...
  output$illustration_WM <- renderPlotly({
    ...
    pWM <- ggplot() +
      ...
      theme(panel.background = element_rect(fill = "transparent"), # bg of the panel
            plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
            axis.text=element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.ticks = element_blank())+
  ...
  output$illustration_pie <- renderPlot({
  ...
  content <- reactive({
    ...
    if (input$HEB_type == 'contrast'){connect_OverlapOrUnique()} else
      if (input$HEB_type == 'trend'){reviewtable.filtered()} else
  ...
  output$illustration_Delay <- renderPlotly({
    ...
    delay1 <- PN_DelayDf_Stack(data=reviewtable.filtered() %>% select(Entry_ID, starts_with('Delay_'), starts_with('Task_type')),
                               split.interval = ' - ')
